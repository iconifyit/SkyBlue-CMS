...
class SqliteDAO extends DAO {
    ...
    function getFieldType($field) {
        ...
        if (strcasecmp(substr($type, 0, 3), 'INT') === 0) {
            $fieldType = "INTEGER";
        }
        else if (strcasecmp(substr($type, 0, 4), 'CHAR') === 0) {
            $fieldType = "CHAR";
        }
        else if (strcasecmp(substr($type, 0, 4), 'TEXT') === 0) {
            $fieldType = "TEXT";
        }
        return $fieldType;
    }
    ...
    function buildUpdateQuery($fields, $Bean) {
        ...
        foreach ($fields as $field) {
            ...
            if (strcasecmp($type, 'INTEGER') !== 0) {
                // PHP 8.2: Properly escape single quotes in string values
