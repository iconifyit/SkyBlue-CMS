...
class MediaDAO extends XmlDAO {
    ...
    function save($data = null) { /* Currently has no save functionality */ }
    ...
    function delete($id) {
        // For MediaDAO, $id should be a Media bean or filepath
        if (is_object($id) && method_exists($id, 'getFilepath')) {
            $filepath = $id->getFilepath();
        } else {
            $filepath = $id;
        }
    ...
    function initDataSource($subpath, $fileName) { return false; }
}