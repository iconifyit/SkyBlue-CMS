...
class SkinDAO extends SqliteDAO {
    ...
    function getFileList() {
        $items = array();
        // SB_SKINS_DIR already contains _SBC_WWW_, so don't concatenate again
        $files = FileSystem::list_dirs(SB_SKINS_DIR, 0);
        $count = count($files);
        for ($i=0; $i<$count; $i++) {
    ...
    function save($data=null) {
        // Disabled - skins cannot be saved via DAO
        return false;
    }

    function copy($TransferObject) {
        // Disabled - skins cannot be copied via DAO
        return false;
    }
