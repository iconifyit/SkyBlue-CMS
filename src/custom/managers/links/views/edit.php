...
<h1 class="h3 mb-3">
    ...
    <span class="text-muted">/</span>
    <a href="admin.php?com=links" class="text-muted text-decoration-none"><?php __('COM.LINKS', 'Links'); ?></a>
    <span class="text-muted">/</span>
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0"><?php echo $Item->getName() ?: __('LINKS.NEW', 'New Link', 1); ?></h5>
        <div>
    ...
    <div class="card-body">
        <div class="row mb-3">
            <div class="col-md-4">
                <label for="id" class="form-label"><?php __('LINKS.ID', 'Item ID'); ?></label>
            ...
            <div class="col-md-4">
                <label for="name" class="form-label"><?php __('LINKS.NAME', 'Name'); ?></label>
                <input class="form-control" type="text" name="name" value="<?php echo $Item->getName(); ?>" id="name" />
            </div>
            <div class="col-md-4">
                <label for="url" class="form-label"><?php __('LINKS.URL', 'URL'); ?></label>
                <input class="form-control" type="text" name="url" value="<?php echo $Item->getUrl(); ?>" id="url" />
            </div>
        </div>

        <div class="row mb-3">
            <div class="col-md-6">
                <label class="form-label"><?php __('LINKS.GROUPS', 'Groups'); ?></label>
                <div class="border rounded p-3">
                    <?php $count = count($groups); ?>
                    <?php for ($i=0; $i<$count; $i++) : ?>
                    <?php $checked = LinksHelper::hasGroup($Item, $groups[$i]) ? " checked" : "" ; ?>
            ...
            <div class="col-md-6">
                <label for="relationship" class="form-label"><?php __('LINKS.REL', 'Relationship'); ?></label>
                <select name="relationship" id="relationship" class="form-select">
                    <option value=""><?php __('GLOBAL.CHOOSE', ' -- Choose -- '); ?></option>
                    <?php echo LinksHelper::getRelationships($Item->getRelationship()); ?>
                </select>
            </div>
        </div>
