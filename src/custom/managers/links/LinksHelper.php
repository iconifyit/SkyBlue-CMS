...
class LinksHelper {
    ...
    // PHP 8.2: Made static to fix non-static method call errors
    static function getLinkDAO($refresh=false) {
        static $Dao;
    ...
    // PHP 8.2: Made static to fix non-static method call errors
    static function getLinksgroupDAO($refresh=false) {
        static $Dao;
        if (! is_object($Dao) || $refresh) {
    ...
    // PHP 8.2: Made static to fix non-static method call errors
    static function getGroups(&$DAO, $refresh=false) {
        global $Core;
        static $groups;
    ...
    // PHP 8.2: Made static to fix non-static method call errors
    static function hasGroup($Item, $Group) {
        return in_array($Group->getId(), $Item->getGroups());
    }

    // PHP 8.2: Made static to fix non-static method call errors
    static function getRelationships($selected="") {
        $values = array(
            'alternate',
            'stylesheet',
            'start',
