...
class LinksController extends Controller {
    ...
    // PHP 8.2: Changed signature to match parent - doCancel only takes $redirect
    function doCancel($redirect = "admin.php?com=links") {
        global $Core;
        ...
        parent::doCancel($redirect);
    }

    // PHP 8.2: Changed signature to match parent - doCancel only takes $redirect
    function doCancelGroups($redirect = "admin.php?com=links&action=listgroups") {
        global $Core;
        $Request = $Core->getRequest();
        $this->updateModelAndView();
        $itemId = $Request->get('id');
    ...
    // PHP 8.2: Added $redirect parameter to match parent signature
    function doSave($Request, $redirect = "admin.php?com=links") {
        ...
        parent::doSave($Request, $redirect);
    }

    // PHP 8.2: Added $redirect parameter to match parent signature
    function doSaveGroup($Request, $redirect = "admin.php?com=links&action=listgroups") {
        $this->addValidation('name', 'notnull', 'LINKS.VALIDATE.NAME');
        parent::doSave($Request, $redirect);
    }

    // PHP 8.2: Added &$Request (by reference) and $redirect to match parent signature
    function doDelete($Request, $redirect = "admin.php?com=links") {
        $itemId = $Request->get('id');
        if (!is_null($itemId)) {
            $this->checkIn(
                $this->dao->getItem($itemId)
        ...
        parent::doDelete($Request, $redirect);
    }

    // PHP 8.2: Added &$Request (by reference) and $redirect to match parent signature
    function doDeleteGroup(&$Request, $redirect = "admin.php?com=links&action=listgroups") {
        $itemId = $Request->get('id');
        if (!is_null($itemId)) {
            $this->checkIn(
                $this->dao->getItem($itemId)
