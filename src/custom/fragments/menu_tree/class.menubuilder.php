...
class MenuBuilder {
    ...
    public function __construct(&$pages, $pid) {
        $this->pid   = $pid;
        $this->pages = $pages;
        $this->menus = array();
    }
    ...
    public function buildTree(&$parent, $pageIdx) {
    ...
    public function loadTree($menuid) {
        global $Authorize;
    ...
    public function renderTree($item, $offset, $depth, $options) {
        global $Core;
        global $Router;
        ...
        if (Filter::get($options, 'li_class', $this->options['li_class'])) {
            $attribs['class'] .= Filter::get($options, 'li_class', $this->options['li_class']);
        }
        if ($item->isActive()) {
        ...
        if (Filter::get($options, 'a_class', $this->options['a_class'])) {
            $attrs['class'] .= ($attrs['class'] ? ' ' : '') . Filter::get($options, 'a_class', $this->options['a_class']);
        }
        $link = HtmlUtils::tag(
            'a',
            $attrs,
