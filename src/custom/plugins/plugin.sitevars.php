...
class SiteVars {
    ...
    function execute($html) {
        ...
        $this->assign('skin.path',            Config::get('site_url') . str_replace(_SBC_WWW_, '', ACTIVE_SKIN_DIR));
        $this->assign('site.map', $this->getSiteMap($pages, $pageId));

        $myVars = $this->getSettingsFile();
        // PHP 8.2: Check if $myVars is an array before using count()
        if (is_array($myVars) && count($myVars)) {
