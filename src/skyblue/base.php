...
// PHP 8.2: Load User class BEFORE session_start() so it can properly unserialize User objects
// Note: TransferObject must be loaded first (done below after defining _INC_)
...
// PHP 8.2: Load User class before session_start() so session can unserialize User objects
// Note: session_start() is NOT called here - it's called by Session->start() when the Session
// object is instantiated later. By loading the User class here, we ensure it's available
// when session_start() eventually gets called.
require_once(_SBC_SYS_ . 'managers/users/User.php');
