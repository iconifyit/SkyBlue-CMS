...
class Core extends Publisher {
    ...
    function __construct($options=array()) {
        file_put_contents('/tmp/frontend-debug.log', "Core::__construct() started\n", FILE_APPEND);
        ...
        file_put_contents('/tmp/frontend-debug.log', "Core::__construct() - about to declareEvents\n", FILE_APPEND);

        $this->declareEvents($options);
        file_put_contents('/tmp/frontend-debug.log', "Core::__construct() - about to LoadConstants\n", FILE_APPEND);
        $this->LoadConstants();
        file_put_contents('/tmp/frontend-debug.log', "Core::__construct() - about to LoadHelperClasses\n", FILE_APPEND);
        # $this->IsValidSite();
        $this->LoadHelperClasses();
        // GetActiveSkin moved to after Config::load() to avoid database constant issues
        // file_put_contents('/tmp/frontend-debug.log', "Core::__construct() - about to GetActiveSkin\n", FILE_APPEND);
        // $this->GetActiveSkin();
        file_put_contents('/tmp/frontend-debug.log', "Core::__construct() - completed\n", FILE_APPEND);
        # $this->InitSession($this->lifetime);
    }
    
    public function declareEvents($config) {
    ...
    public function register($event, $callback, $priority=0) {
        Event::register($event, $callback, $priority);
    }
    ...
    public function trigger($event, $html=null) {
    ...
    public function editor($selector, &$html, $editor) {
        $code = null;
    ...
    public function hasSelector($selector, &$html) {
        $attr = 'class';
        if ($selector[0] == '#') {
            $attr = 'id';
        }
    ...
    public function FileNotFound($missingFile, $line, $func) {
    ...
    public function DoInclude($file, $lineNumber, $funcName) {
        if (file_exists($file)) {
            require_once($file);
        } 
        else {
    ...
    public function LoadConfig() {/* Deprecated */}
    ...
    public function LoadLanguage() {/* Deprecated */}
    ...
    static function LoadPlugin($plugin) {
        ...
        if (!empty($plugin)) {
            ...
            if (file_exists($path)) {
                ...
                trigger_error(
                    'SkyBlue CMS Says: File '.$plugin.' does not exist. '
            ...
            trigger_error(
                'SkyBlue CMS Says: No Plugin specified. '
                . __FILE__ . ': on line ' . __LINE__
            );
        }
    }
    ...
    public function RequireID($id, $redirect) {
        if (empty($id)) {
    ...
    public function ExitEvent($code, $redirect) {
        $msg = null;
        $class = null;
    ...
    public function ExitDemoEvent($redirect) {
        $this->SetSessionMessage(MSG_FEATURE_DISABLED_IN_DEMO, 'warning');
        Utils::redirect($redirect);
    }
    
    public function ExitRestrictedEvent($redirect, $msg) {
        $this->SetSessionMessage($msg, 'warning');
        Utils::redirect($redirect);
    }
    
    public function ExitWithWarning($msg, $redirect) {
        $this->SetSessionMessage($msg, 'warning');
        Utils::redirect($redirect);
    }
    
    public function ExitWithErrorMessage($msg, $redirect) {
        $this->SetSessionMessage($msg, 'error');
        Utils::redirect($redirect);
    }
    
    public function ExitWithError($msg, $redirect) {
    ...
    public function DefineDefaultPage() {
    ...
    public function PageSelector($selected='') {
        $pages = get_pages();
        $options = array(
            HtmlUtils::option(
    ...
    public function BuildFileSelector($dir, $selector_name, $selected='', $trimext=0, $fullpath=1) {
    ...
    public function ValidateField($value, $validation) {
        switch ($validation) {
            ...
            case 'notnull':
                ...
                break;
            case 'number':
                return preg_match('/' . SB_REGEX_NUM . '/', $value);
                break;
            case 'email':
                return preg_match('/' . SB_REGEX_EMAIL . '/i', $value);
                break;
            case 'url':
    ...
    public function CalcNumOfPages(&$items, $items_per_page) {
    ...
    public function AlphaNumericFilter($value) {
    ...
    public function SBRedirect($url) {
        Utils::redirect($url);
    }
    ...
    public function SafeURLFormat($str) {
    ...
    public function GetLink($title, $PageID, $ObjID, $useFullURL=0) {
    ...
    public function InitSession($lifetime=SB_SESSION_LIFETIME) {
        $Session =& Singleton::getInstance('Session');
    }
    ...
    public function SetSessionMessage($msg, $type='confirm') {
        $class = 'generic';
        $heading = null;
    ...
    public function LoadContent($mgr) {
        $Manager = $mgr;
    ...
    public function LoadModuleAdmin2($mod) {
    ...
    public function InitDataSource($src, $objs, $type) {
        if (!file_exists($src)) {
    ...
    public function TrimExtension($file) {
    ...
    public function SubDirFromPath($path, $index) {
        $dirs = explode('/', $path);
    ...
    public function ListDirsToLevel($dir, $dirs=array(), $depth='', $lvl=0) {
        $lvl++;
    ...
    public function GetParentProperty($objs, $obj, $key, $default) {
        $property = $default;
        if (isset($obj->$key) && 
             $obj->$key != 'null' && 
             trim($obj->$key) != '')
    ...
    public function RadioOption($name, $value, $label, $checked=0) {
    ...
    public function RadioSelector($options) {
    ...
    public function MakeOption($title, $value, $selected='') {
        $sel = '';
        if ($selected == 1) {
    ...
    public function SelectorOptions($opts, $selected=null) {
        $res = array();
        foreach ($opts as $k=>$v) {
    ...
    public function MakeOptionGroup($options, $label) {
    ...
    public function Selector($name, $keyValuePairs, $selected=null) {
    ...
    public function SelectList($arr, $name, $size=1, $js='') {
    ...
    public function YesNoList($name, $selected=1) {
        $options = array();
    ...
    public function MeridianSelector($meridian='') {
        $opts = array();
        $s = $meridian == 'AM' ? 1 : 0 ;
    ...
    public function GetTokenList($str) {
          preg_match_all(
    ...
    public function GetPageRegions($str) {
          preg_match_all(
    ...
    public function RegexGetTokens($pattern, $str) {
          preg_match_all(
    ...
    public function MakeTokenSelector($skin, $name, $selected='') {
    ...
    public function OrderSelector($objs, $key, $value) {
        $selector     = '';
        if (count($objs) > 1) {
            $options = array();
    ...
    public function ImageSelector($selname, $subdir, $match='', $js='') {
    ...
    public function OrderSelector2($objs, $key, $value) {
    ...
    public function UploadFile($file, $dest, $allowtypes, $maxsize=5000000, $targets=array()) {
    ...
    public function CalcStartOfRange($offset, $item_count, $min=0) {
    ...
    public function GetNumInRange($num, $max, $min=1) {
    ...
    public function GetVar($arr, $index, $default, $htmlfilter=1, $alphafilter=0) {
    ...
    public function OutputBuffer($input) {
    ...
    public function SBReadFile($file) {
    ...
    public function WriteFile($file, $str, $append=0) {
    ...
    public function ListFiles($dir, $files=array()) {
    ...
    public function ListFilesOptionalRecurse($dir, $recurse=1, $files=array()) {
    ...
    public function ListDirs($dir, $dirs=array()) {
    ...
    public function ListDirsOptionalRecurse($dir, $recurse=1, $dirs=array()) {
    ...
    public function ExecNukeDir($dir, $ContentsOnly=1) {
    ...
    public function ExecCopyDir($from, $to) {
        return FileSystem::copy_dir($from, $to);
    }
    ...
    public function ArrayToMatrix($arr, $colcount=3) {
    ...
    public function ArrayToGrid($arr, $colcount=3) {
    ...
    public function CountObjs($objs, $key, $value) {
    ...
    public function ArrayToObj($obj, $arr) {
    ...
    public function UpdateObjFromArray($obj, $arr) {
    ...
    public function SelectObj($objs, $id) {
    ...
    public function SelectObjByKey($objs, $key, $match) {
    ...
    public function SelectObjsByKey($objs, $key, $match) {
    ...
    public function InsertObjByKey($obj, $objs, $key, $match) {
    ...
    public function SelectItemFromTree($objs, $key, $match, $children) {
    ...
    public function ObjInArray($obj, $array, $key) {
    ...
    public function SelectObjs($objs, $k, $v) {
    ...
    public function InsertObj($objs, $obj, $match) {
    ...
    public function OrderObjs($objs, $id, $index) {
    ...
    public function DeleteObj($objs, $id) {
    ...
    public function SBStrReplace($haystack, $needle, $replace) {
    ...
    public function ImageDimsToMaxDim($dims, $maxwidth, $maxheight) {
    ...
    public function ImageHeight($fp) {
    ...
    public function TrimArrayItems($arr) {
    ...
    public function OffsetInArray($arr, $match) {
    ...
    public function GetArraySubset(&$arr, $offset, $item_count) {
    ...
    public function StateSelector($selected=null) {
