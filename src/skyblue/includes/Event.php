...
class Event {
    ...
    static function & events($event=null, $values=array()) {
        static $_events;
        if (!is_array($_events)) {
            $_events = array();
        }
        if (!empty($event) && !isset($_events[$event])) {
            $_events[$event] = array();
        }
    ...
    static function addEvent($event) {
    ...
    static function hasCallbacks($event) {
    ...
    static function isRegistered($event) {
    ...
    static function unregister($event, $callback) {
        if (!Event::isRegistered($event)) return;
     ...
     * @param mixed  The data arguments on which to operate (if any)
     * @return mixed
     */
    static function trigger($event, $data=null): mixed {
        if (Event::isRegistered($event)) {
            $_events =& Event::events();
    ...
    static function triggerOnce($event, $data=null) {
        if (Event::isRegistered($event)) {
            $_events =& Event::events();
            $count = count($_events[$event]);
            for ($i=0; $i<$count; $i++) {
