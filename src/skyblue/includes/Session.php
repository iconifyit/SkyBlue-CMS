...
class Session extends Publisher {
    ...
    function start($options=array()) {
        ...
        // PHP 8.2: Only configure session if it hasn't been started yet
     ...
     */
    function getUser() {
        // PHP 8.2: Objects stored in session are already properly serialized/unserialized
        // Don't create a new User object - just return the existing one
        $user = $this->get('User');
        if (is_object($user)) {
            return $user;
        }
        // Fallback for old behavior if User is stored as array
