...
class FileSystem extends SkyBlueObject {
     ...
     */
    static function buffer($file, $data=null) {
    ...
    /**
     * Reads a SkyBlue CMS config ini file
     * @param        The file path
     * @return       An INI array
     */
    static function read_config($file) {
     ...
     * @return       An INI array
     */
    static function parse_config_string($config) {
        $ini = array();
        if (!empty($config)) {
            $lines = explode("\n", trim($config));
     ...
     */
    static function write_file($file, $str, $mode='w+') {
     ...
     */
    static function move_file($from, $to) {
        if (!file_exists($to)) {
     ...
     */
    static function list_files($dir, $recurse=0, $files=array()) {
     ...
     */
    static function list_dirs($dir, $recurse=1, $dirs=array()) {
        $dir = Utils::checkTrailingSlash($dir);
        ini_set('max_execution_time', 10);
        if (!is_dir($dir)) {
            die ("$dir is not a valid directory in " . __METHOD__);
     ...
     */
    static function copy_dir($srcdir, $dstdir) {
        if (!is_dir($srcdir))
            return false;
        if (!is_dir($dstdir))
            mkdir($dstdir);
     ...
     * @return bool  Whether or not the file was deleted
     */
    static function delete_files($dir, $delDir = false) {
        if (!$dh = @opendir($dir))
            return;
     ...
     */
    static function delete_dir($dir, $contentsOnly=true) {
        if (!is_dir($dir)) {
            return false;
        }
     ...
     */
    static function file_time($file) {
        if (file_exists($file)) {
     ...
     */
    static function parse_config_file($file) {
     ...
     */
    static function touch_file($file) {
        return touch($file);
    }
     ...
     */
    static function chown($file, $uid) {
     ...
     */
    static function chgrp($file, $gid) {
        if (!file_exists($file)) return false;
     ...
     */
    static function chmod($file, $mod) {
        if (!file_exists($file)) return false;
     ...
     */
    static function safe_file_name($name) {
     ...
     */
    static function file_type($file) {
     ...
     */
    static function file_perms($file) {
        if (!file_exists($file)) {
            return '000';
        }
        $n = is_dir($file) ? 2 : 3 ;
