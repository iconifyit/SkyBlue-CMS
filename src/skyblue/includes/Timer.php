...
/**
 * A static class for tracking performance of tasks in SkyBlue CMS
 *
 * Usage:
 *
 *    Timer::start('myevent');
...
class Timer {
    ...
    static function & timers($event=null, $values=array()) {
        static $_timers;
        if (!is_array($_timers)) {
            $_timers = array();
        }
    ...
    static function setStartTime($event, $time) {
        Timer::timers($event, array(
            'start' => $time,
            'stopped' => false
        ));
    ...
    static function setEndTime($event, $time) {
        Timer::timers($event, array(
            'stop' => $time,
            'stopped' => true
        ));
    ...
    static function isStopped($event) {
        return Filter::get(
    ...
    static function getStartTime($event) {
        $timer = Filter::get(Timer::timers(), $event);
        if (!empty($timer)) {
    ...
    static function getEndTime($event) {
        $timer = Filter::get(Timer::timers(), $event);
        if (!empty($timer)) {
