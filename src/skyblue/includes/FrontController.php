...
class FrontController extends Publisher {
    ...
    function __construct(&$Request, $options=array()) {
        ...
        MVC::loadBeanClass($class, $rootpath);
        MVC::loadViewClass($class, $rootpath);
        MVC::loadDaoClass($class, $rootpath);
        MVC::loadControllerClass($class, $rootpath);
    ...
    function display($sendHeaders=true) {
        ...
        $output = Event::trigger('system.display', $result);
        echo $output;
    }
    ...
    static function handleError($errno, $errstr, $errfile, $errline) {
        ...
        if ($fatal && Config::get("display_errors")) {
            $Model = new Model("");
            $Model->setData(new SkyBlueError(
                $type,
                $errno,
                $errstr,
                $errfile,
