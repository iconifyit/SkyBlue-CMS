...
class HtmlUtils {
     ...
     */
    static function yesNoList($name, $selected=1) {
     ...
     */
    static function meridianSelector($selected=null) {
        return HtmlUtils::selector(array(
     ...
     */
    static function option($title, $value, $selected='') {
        $sel = '';
        if ($selected == 1) {
     ...
     */
    static function tag($name, $attrs=array(), $cdata='', $hasCloseTag=1) {
        $attrs_str = null;
        if (count($attrs)) {
            $attrs_str = "";
     ...
     */
    static function mgrButton($value, $action, $customAttrs=array()) {
        $attrs = array(
            'type'  => 'submit',
            'name'  => 'submit',
     ...
     * @return void
     */
    static function mgrButtonCancel() {
     ...
     * @return void
     */
    static function mgrFormOpen($managerName, $customAttrs=array()) {
        $attrs = " ";
        if (count($customAttrs)) {
            foreach ($customAttrs as $key=>$value) {
                $attrs .= "{$key}=\"{$value}\"";
     ...
     */
    static function mgrTasks($i, $objCount, $object, $tasks, $params=array(), $itemsPerPage=10) {
     ...
     * @return void
     */
    static function mgrTaskDelete($com, $id, $name='', $customAttrs=array(), $params=array()) {
        $url_params = "";
        if (count($params)) {
     ...
     * @return void
     */
    static function mgrTaskEdit($com, $id, $name='', $customAttrs=array(), $params=array()) {
        $url_params = "";
        if (count($params)) {
            foreach ($params as $key=>$value) {
                $url_params .= "&{$key}={$value}";
     ...
     * @return void
     */
    static function mgrTaskCopy($com, $id, $name='', $customAttrs=array(), $params=array()) {
        $url_params = "";
        if (count($params)) {
            foreach ($params as $key=>$value) {
                $url_params .= "&{$key}={$value}";
     ...
     * @return void
     */
    static function mgrTask($com, $action, $id, $name='', $icon='', $customAttrs=array(), $params=array()) {
     ...
     * @return void
     */
    static function field($name, $value, $customAttrs=array()) {
        $attrs = array(
            'type'  => 'text',
            'name'  => $name,
            'value' => $value, 
     ...
     * @return void
     */
    static function label($text, $for='', $customAttrs=array()) {
        $attrs = array();
        if (!empty($for)) {
            $attrs = array('for' => $for);
        }
     ...
     */
    static function formatMessage($Message, $closable=true) {
