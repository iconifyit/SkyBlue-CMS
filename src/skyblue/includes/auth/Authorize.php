...
class Authorize extends Publisher {
    ...
    function check(&$Controller, $action) {
        ...
        // Pass if ACLs are globally disabled (check this FIRST before anything else)
        ...
        error_log("[Authorize::check] User: " . (is_object($User) ? get_class($User) : gettype($User)));
        error_log("[Authorize::check] User ID: " . (is_object($User) && method_exists($User, 'getId') ? $User->getId() : 'N/A'));
        error_log("[Authorize::check] User username: " . (is_object($User) && method_exists($User, 'getUsername') ? $User->getUsername() : 'N/A'));

        $Aco = $this->_findAco(get_class($Controller));

        if (!is_object($Aco)) {
            error_log("[Authorize::check] Aco not found for: " . get_class($Controller));
            return false;
        }
        ...
        if ($this->enabled) {
            if (empty($User)) {
                error_log("[Authorize::check] User is empty");
                return false;
            }
            if (trim($User->getUsername()) == "") {
                error_log("[Authorize::check] Username is empty");
                return false;
            }
        }
