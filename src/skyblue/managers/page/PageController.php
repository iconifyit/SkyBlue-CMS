...
class PageController extends Controller {
    ...
    function doDelete($Request, $redirect = null) {
        if (! is_admin_page()) Utils::redirect("admin.php?com=login&action=logout");
        ...
        if ($Page->getIsdefault()) {
            ...
            Utils::redirect($redirect ?: "admin.php?com=page");
        }
        else {
            $itemId = $Request->get('id');
            if (! is_null($itemId)) {
            ...
            Utils::redirect($redirect ?: "admin.php?com=page&pageNum={$Request->get('pageNum', 1)}");
        }
    }
     ...
     */
    function doSave($Request, $redirect = null) {
        ...
        if ($this->validate($Request)) {
            ...
            $redirect = $redirect ?: "admin.php?com=page&pageNum={$Request->get('pageNum', 1)}";
     ...
     * @param string $redirect  The redirect URL (optional, uses Request for page info if not provided)
     * @see includes/mvc/Controller#doCancel($redirect)
     */
    function doCancel($redirect = null) {
        // Get Request object from controller
