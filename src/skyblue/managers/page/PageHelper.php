...
class PageHelper {
    ...
    static function getPages($refresh=false) {
        static $pages;
    ...
    static function getPermalink($Page) {
        global $Router;
    ...
    static function buildXmlTree($nodes) {
        $tree = "";
        if ($nodes->length) {
    ...
    static function buildHtmlTree($pages) {
        $tree = "";
        if ($pages->length) {
    ...
    static function getTreeView($data, $options=array()) {
        $tree = "";
        try {
    ...
    static function buildSiteTree($pages, $options=array()) {
        $tree = "";
        if ($pages->length) {
    ...
    static function getElementById($doc, $id) {
        $xpath = new DOMXPath($doc);
        return $xpath->query("//*[@id='$id']")->item(0);
    }

    // PHP 8.2: Made static to fix non-static method call errors
    static function parseStructureXml() {
        $Dom = null;
        try {
            $DaoClass = ucwords(DB_TYPE) . "DAO";
            $Dao = new $DaoClass(array(
    ...
    // PHP 8.2: Made static to fix non-static method call errors
    static function saveStructureXml($xml) {
        $result = false;
        try {
            $DaoClass = ucwords(DB_TYPE) . "DAO";
            $Dao = new $DaoClass(array(
     ...
     */
    static function getUniquePermalink($Page, $permalink) {
        global $Router;
     ...
     */
    static function getPageTypeOptions() {
        static $types;
        if (!is_array($types)) {
            $types = array();
            // PHP 8.2: Check if ACTIVE_SKIN_DIR is defined before using it
            if (defined('ACTIVE_SKIN_DIR')) {
     ...
     */
    static function getMenuListOptions() {
        static $menus;
        if (! is_array($menus)) {
     ...
     */
    static function getParentPageOptions(&$Page) {
        global $pages;
        if (!is_array($pages)) {
            $pages = array();
    ...
    static function AclTypeSelector($selected='') {
    ...
    // PHP 8.2: Made static to fix non-static method call errors
    static function getUserFullName($username) {
    ...
    static function getAuthorSelector($author='') {
        global $Authenticate;
    ...
    static function UserSelector($users=null) {
        global $Core;
    ...
    static function UserGroupsSelector($groups=null) {
        global $Core;
