...
class PluginHelper {
    ...
    static function findElement($doc, $key, $value) {
        $node = null;
        $xpath = new DOMXPath($doc);
        try {
    ...
    static function parseStructure() {
        $Dom = null;
        try {
    ...
    static function saveStructure($xml) {
        try {
            $DaoClass = ucwords(DB_TYPE) . "DAO";
            $Dao = new $DaoClass(array(
                'type' => 'structure',
    ...
    static function getDataFile($Bean) {
        $dataFile = $Bean->getDatafile();
    }

    static function getBeanByKey($key, $value, $refresh=false) {
        static $Bean;
